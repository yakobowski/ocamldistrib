...
let is_crt_lib = function "LIBCMT" | "MSVCRT" -> true | _ -> false
...
let temp_file s x = add_temp (Filename.temp_file s x)

let open_temp_file s x =
  let f, c = Filename.open_temp_file s x in
  (add_temp f, c)

let safe_remove s = try Sys.remove s with Sys_error _ -> ()

let () =
  at_exit (fun () -> if not !save_temps then List.iter safe_remove !temps)
...
let get_output ?(use_bash = false) ?(accept_error = false) cmd =
  ...
  ( if String.length cmd' < 8182 && not use_bash then (
    if Sys.command cmd' <> 0 && not accept_error then
      failwith ("Cannot run " ^ cmd) )
  else
    let cfn, oc = open_temp_file "longcmd" ".sh" in
    output_string oc cmd' ;
    close_out oc ;
    if Sys.command (Printf.sprintf "bash %s" cfn) <> 0 then
      failwith ("Cannot run " ^ cmd) ) ;
...
let mk_dirs_opt pr =
  String.concat " " (List.map (fun s -> pr ^ Filename.quote s) !dirs)
...
let build_diversion lst =
  ...
  let utf16, lst =
    ...
    | lst -> (true, lst)
    | exception Not_utf8 -> (false, lst)
  in
  if utf16 then output_string oc "\xFF\xFE" ;
  (* LE BOM *)
  List.iter (fun s -> output_string oc s) lst ;
  close_out oc ;
  "@" ^ responsefile

type cmdline = {may_use_response_file: bool}
...
let run_command cmdline cmd =
  ...
  else if Sys.command cmd_quiet <> 0 then (
...
let quote_files cmdline lst =
  ...
  in
  if String.length s >= 1024 && cmdline.may_use_response_file then
    Filename.quote (build_diversion lst)
  else s
...
let cygpath l =
  get_output
    (Printf.sprintf "cygpath -m %s"
       (String.concat " " (List.map Filename.quote l)))

let cygpath1 fn = get_output1 (Printf.sprintf "cygpath -m %s" fn)

let file_exists fn =
  if Sys.file_exists fn && not (Sys.is_directory fn) then Some fn
  else if !use_cygpath && Sys.file_exists (fn ^ ".lnk") then Some (cygpath1 fn)
  else None

let rec find_file_in = function
  | [] -> None
  | fn :: rest -> (
    match file_exists fn with Some x -> Some x | None -> find_file_in rest )
...
let rec map_until_found f = function
  | [] -> None
  | x :: xs -> ( match f x with None -> map_until_found f xs | r -> r )
...
let find_file =
  ...
  fun fn ->
    ...
    try Hashtbl.find memo k with Not_found -> (
      try Hashtbl.find memo (k ^ ".lib") with Not_found ->
        let fns =
        ...
        let r =
          ...
          | Some fn -> fn
          | None -> failwith (Printf.sprintf "Cannot find file %S" fn)
        in
...
let exportable s =
  ...
  | `x86 -> s <> "" && (s.[0] = '_' || s.[0] = '?')
  | `x64 ->
      if String.length s > 2 && s.[0] = '?' && s.[1] = '?' then false else true
...
let drop_underscore obj s =
  ...
  | `x86 -> (
      ...
      | _ ->
          failwith
            (Printf.sprintf "In %s, symbol %s doesn't start with _ or ?"
               obj.obj_name s) )
  | `x64 -> s
...
let add_reloc_table obj obj_name p =
  let sname = Symbol.gen_sym () in
  (* symbol pointing to the reloc table *)
  ...
  let reloc secsym min max rel =
    if p rel.symbol then (
      ...
      let kind =
        ...
        | ( `x86
          , ( 0x0a (* IMAGE_REL_I386_SECTION *) | 0x0b
            (* IMAGE_REL_I386_SECREL*) ) ) ->
            0x0100 (* debug relocs: ignore *)
        | _, k ->
            let msg =
              Printf.sprintf "Unsupported relocation kind %04x for %s in %s" k
                rel.symbol.sym_name obj_name
            in
            failwith msg
        (*            Printf.eprintf "%s\n" msg;
            0x0001 *)
      ...
      let pos =
        try Hashtbl.find str_pos name with Not_found ->
      ...
      int_to_buf data pos ;
      Reloc.abs !machine sect
        (Int32.of_int (Buffer.length data))
        (Lazy.force secsym) ;
  ...
  let section sec =
    if
      sec.sec_opts &&& 0x1000l <> 0l
      && has_prefix ".rdata$.refptr." sec.sec_name
    then (
      ...
      sec.sec_name
      <- Printf.sprintf ".flexrefptrsection%i" (Oo.id (object end)) ) ;
    let min = ref Int32.max_int and max = ref Int32.min_int in
    let sym =
      lazy
        (let s = Symbol.intern sec 0l in
         syms := s :: !syms ;
         s)
    in
  ...
  <- Symbol.export sname sect 0l
     :: strsym :: nonwrsym
     :: List.filter (fun x -> not (p x)) obj.symbols
     @ !syms ;
  sname
...
let add_import_table obj imports =
  ...
  List.iter
    (fun s ->
      ...
      obj.symbols
      <- sym
         :: Symbol.export ("__imp_" ^ s) sect (Int32.of_int !i)
         :: obj.symbols ;
...
let collect_dllexports obj =
  ...
  let l =
    List.map
      (function _, x :: _ -> x | _ -> assert false)
      (List.find_all
         (fun (cmd, _args) -> String.uppercase_ascii cmd = "EXPORT")
         dirs)
  in
  match !toolchain with
...
let parse_dll_exports fn =
  ...
  let exps = ref [] in
  try
    while input_line ic <> "[Ordinal/Name Pointer] Table" do
      ()
    done ;
    while true do
    ...
    assert false
  with
  | Not_found | End_of_file -> close_in ic ; !exps
...
let dll_exports fn =
  ...
  | `GNAT | `CYGWIN | `CYGWIN64 | `MINGW | `MINGW64 ->
      ...
      if cmd_verbose (Printf.sprintf "%s -p %s > %s" !objdump fn dmp) <> 0 then
        failwith "Error while extracting exports from a DLL" ;
  ...
  | Some x -> (
      let filename =
        if
          ...
          && Sys.file_exists (filename ^ ".exe")
        then filename ^ ".exe"
        else filename
      in
      let filename = if !use_cygpath then cygpath1 filename else filename in
      try Stacksize.set_stack_reserve filename x with exn ->
        Printf.eprintf "Cannot set stack reserve: %s" (Printexc.to_string exn)
      )
  | None -> ()
...
let needed imported defined unalias obj =
  ...
  List.fold_left
    (fun accu sym ->
      if Symbol.is_extern sym then
        StrSet.add (normalize_imp (normalize sym.sym_name)) accu
      else accu )
    StrSet.empty obj.symbols
...
let build_dll link_exe output_file files exts extra_args =
  ...
  let read_file fn =
    if Lib.is_dll fn then `Lib ([], dll_exports fn) else Lib.read fn
  in
  ...
  let libs =
    collect (function f, `Lib (x, _) -> Some (f, x) | _ -> None) files
  in
  ...
  let defined, from_imports, unalias =
    ...
    let rec collect_defined_obj obj =
      ...
      List.iter
        (fun sym ->
          if has_prefix ".refptr." sym.sym_name then
            sym.sym_name
            <- Printf.sprintf "??flexrefptr%i" (Oo.id (object end)) )
        obj.symbols ;
      (* Collect aliases *)
      List.iter
        (fun (x, y) ->
      ...
      let register_deflib fn =
        if not (is_crt_lib fn) then
          let fn = find_file fn in
          if not (Hashtbl.mem loaded_filenames fn) then (
            Hashtbl.add loaded_filenames fn () ;
            collect_file fn )
      in
      if (not !builtin_linker) && !use_default_libs && not !custom_crt then
        List.iter
          (fun (cmd, args) ->
            if String.uppercase_ascii cmd = "DEFAULTLIB" then
              List.iter register_deflib args )
          (Coff.directives obj) ;
      ...
      List.iter
        ...
        obj.symbols
    and collect_file fn =
      if not (Hashtbl.mem collected (String.lowercase_ascii fn)) then (
    ...
    if !machine = `x64 then add_def "__ImageBase" else add_def "___ImageBase" ;
  ...
  let imports obj =
    ...
    imported_from_implib :=
      StrSet.union !imported_from_implib (StrSet.inter n from_imports) ;
  ...
  let record_obj obj =
    if !builtin_linker then ""
      ...
      Coff.put oc obj ; close_out oc ; fn
  in
  ...
  let close_obj name imps obj =
    error_imports name imps ; add_reloc name obj imps ; record_obj obj
  in
  let dll_exports = ref StrSet.empty in
  let rec link_obj fn obj =
    List.iter
      (fun sym ->
        if Symbol.is_defin sym && exportable sym.sym_name then
          exported := StrSet.add sym.sym_name !exported )
      obj.symbols ;
    dll_exports :=
      List.fold_left
        (fun accu x -> StrSet.add x accu)
  ...
  Hashtbl.iter
    (fun (libname, objname) (obj, imps) ->
      if StrSet.is_empty imps then Hashtbl.replace need_lib libname ()
...
*)
  ...
  if !builtin_linker then (
    ...
    | Some x when not !dry_mode ->
        let fn =
          if x = "" then Filename.chop_extension output_file ^ ".def" else x
        in
    ...
    let cmd =
      ...
      | `MSVC | `MSVC64 ->
          ...
          let implib =
            if !implib then Filename.chop_extension output_file ^ ".lib"
            else temp_file "dyndll_implib" ".lib"
          in
          (* VS 2017.3 doesn't seem to be able to cope with /implib: existing but
           being an empty file. *)
          let c = open_out implib in
          output_string c "x" ;
          close_out c ;
          let _impexp =
            add_temp (Filename.chop_suffix implib ".lib" ^ ".exp")
          in
          let extra_args =
            if !custom_crt then
              "/nodefaultlib:LIBCMT /nodefaultlib:MSVCRT " ^ extra_args
            else "msvcrt.lib " ^ extra_args
          in
          let extra_args =
            if !machine = `x64 then
              Printf.sprintf "/base:%s " !base_addr ^ extra_args
            else extra_args
          in
          ...
          Printf.sprintf
            ...
            (mk_dirs_opt "/libpath:") (Filename.quote implib)
            (Filename.quote output_file)
            !subsystem files descr extra_args
      | `CYGWIN | `CYGWIN64 ->
          let def_file =
            if main_pgm then ""
            else
          ...
          let extra_args =
            if !machine = `x64 then
              "-Xlinker --image-base -Xlinker 0x10000 " ^ extra_args
            else extra_args
          in
          Printf.sprintf "%s %s%s -L. %s %s -o %s %s %s %s %s" !gcc
            (if link_exe = `EXE then "" else "-shared ")
            ( if main_pgm then ""
            else if !noentry then "-Wl,-e0 "
            else if !machine = `x86 then "-Wl,-e_FlexDLLiniter@12 "
            else "-Wl,-eFlexDLLiniter " )
            (mk_dirs_opt "-I") (mk_dirs_opt "-L")
            (Filename.quote output_file)
      ...
      | `MINGW | `MINGW64 | `GNAT ->
          let def_file =
            if main_pgm then ""
              ...
              Filename.quote def_file
          in
          Printf.sprintf "%s -m%s %s%s -L. %s %s -o %s %s %s %s %s %s" !gcc
            !subsystem
            (if link_exe = `EXE then "" else "-shared ")
            ( if main_pgm then ""
            else if !noentry then "-Wl,-e0 "
            ...
            (mk_dirs_opt "-I") (mk_dirs_opt "-L")
            (Filename.quote output_file)
            descr files def_file
            ( if !implib then
              "-Wl,--out-implib="
              ^ Filename.quote (Filename.chop_extension output_file ^ ".a")
            else "" )
            extra_args
      | `LIGHTLD ->
          no_merge_manifest := true ;
          Printf.sprintf "ld %s%s -o %s %s %s %s %s"
            (if link_exe = `EXE then "" else "--shared ")
            ( if main_pgm then ""
            else if !noentry then "-e0 "
            else "-e FlexDLLiniter@12 " )
            (Filename.quote output_file)
            descr files
            ( if !implib then
              "--out-implib "
    ...
    if not !dry_mode then (
      ...
      if
        (not !no_merge_manifest) && !merge_manifest
        && ((not !real_manifest) || Sys.file_exists manifest_file)
      then (
        let fn =
        ...
        let mcmd =
          Printf.sprintf "mt -nologo -outputresource:%s -manifest %s"
            (Filename.quote
               (if link_exe = `EXE then output_file else output_file ^ ";#2"))
            (Filename.quote fn)
        in
        if !verbose >= 1 then Printf.printf "+ %s\n%!" mcmd ;
...
let read_gnatls () =
  ...
  let ada_include =
    List.hd (List.filter (fun s -> ends_with s "adainclude") str_l)
  in
  ...
  if str = "" then []
    ...
    let rec loop acc pos =
      if pos > String.length str then List.rev acc
        ...
        let i =
          try String.index_from str pos sep with Not_found -> String.length str
        in
...
let remove_duplicate_paths paths =
  ...
  let rec loop paths =
    match paths with
    | path :: paths -> (
      try Hashtbl.find set path ; loop paths with Not_found ->
        Hashtbl.add set path () ; path :: loop paths )
    | [] -> []
  in
  loop paths
...
let setup_toolchain () =
  let mingw_libs pre =
    ...
    let rec get_lib_search_dirs input =
      ...
      | entry :: input -> (
        ...
        with Not_found -> get_lib_search_dirs input )
      | [] -> []
    in
    let lib_search_dirs =
      get_output (!gcc ^ " -print-search-dirs")
      |> get_lib_search_dirs |> List.map normalize_path
  ...
  | `CYGWIN | `CYGWIN64 ->
      ...
      search_path :=
        ...
        @ [ "/lib"
          ; "/lib/w32api"
          ; Filename.dirname
              (get_output1 ~use_bash:true "gcc -print-libgcc-file-name") ] ;
      default_libs :=
        [ "-lkernel32"
        ; "-luser32"
        ; "-ladvapi32"
  ...
  | `MSVC | `MSVC64 ->
      search_path :=
        !dirs @ parse_libpath (try Sys.getenv "LIB" with Not_found -> "") ;
      if not !custom_crt then default_libs := ["msvcrt.lib"]
  | `MINGW -> mingw_libs Version.mingw_prefix
  | `MINGW64 -> mingw_libs Version.mingw64_prefix
  | `GNAT ->
  ...
  | `LIGHTLD -> search_path := !dirs

let display_msvc_output file name =
  let c = open_in file in
  try
    let first = input_line c in
    if first <> Filename.basename name then print_string first ;
    while true do
...
let compile_if_needed file =
  if Filename.check_suffix file ".c" then (
    ...
    let pipe, stdout =
      if
        (!toolchain = `MSVC || !toolchain = `MSVC64)
        && !verbose < 2 && not !dry_mode
      then
        try
    ...
    let cmd =
      match !toolchain with
      | `MSVC | `MSVC64 ->
          Printf.sprintf "cl /c /MD /nologo /Fo%s %s %s%s"
            (Filename.quote tmp_obj) (mk_dirs_opt "/I") file pipe
      | `CYGWIN | `CYGWIN64 ->
          Printf.sprintf "gcc -c -o %s %s %s" (Filename.quote tmp_obj)
            (mk_dirs_opt "-I") file
      | `MINGW | `MINGW64 | `GNAT ->
          Printf.sprintf "%s -c -o %s %s %s" !gcc (Filename.quote tmp_obj)
            (mk_dirs_opt "-I") (Filename.quote file)
      | `LIGHTLD ->
...
let all_files () =
  ...
  let f obj =
    ...
    if file_exists fn <> None then fn else obj
  in
  let tc =
    match !toolchain with
    | `MSVC -> "msvc.obj"
  ...
  if !exe_mode <> `DLL then
    if !add_flexdll_obj then f ("flexdll_" ^ tc) :: files else files
  else if !noentry then files
...
let main () =
  ...
  (use_cygpath :=
     ...
     | (`GNAT | `MINGW | `MINGW64 | `CYGWIN | `CYGWIN64), `None -> (
       ...
       | "Win32" -> Sys.command "cygpath -S 2>NUL >NUL" = 0
       | _ -> assert false )
  ...
  | `PATCH -> patch_output !output_file
